...
#define PLAYERHEIGHT 170
#define MAXSPEEDY 1000.f
#define MAXSPEEDX 1000.f
#define JUMPSPEED -850.f
#define GRAVITY 1300.f
#define HORIZONTAL_ACCELERATION 4800.f
#define HORIZONTAL_FRICTION 1920.f
#define HORIZONTAL_ACCELERATION_AIR 800.f
...
void CollisionTest::Update(Uint32 ticks)
{
    ...
    if (m_input.KeyPressed(SDL_SCANCODE_D))
    {
    ...
    if (m_update)
    {
        ...
        bool falling = m_velocity.m_y > 0.0f;
    ...
    m_cameraX = static_cast<int>(m_position.m_x) - 640; // max(MINCAMERAX, min(MAXCAMERAX,
...
void CollisionTest::Draw(SDL_Renderer* renderer)
{
    ...
    if (m_diagnostics)
    {
        SDL_Rect playerRect = { static_cast<int>(m_position.m_x - PLAYERWIDTH * 0.5f), static_cast<int>(m_position.m_y - PLAYERHEIGHT), PLAYERWIDTH,
                                PLAYERHEIGHT };
...
void CollisionTest::clip_move(Vector2& move)
{
    Vector2 new_position;
    new_position.Add(m_position, move);

    if (!colliding(new_position))
    {
        return;
    }

    Vector2 new_move = move;
    new_move.m_x = 0.0f;
    new_position.Add(m_position, new_move);
    ...
    move.m_x = move.m_y = 0.0f;
    m_velocity.m_x = m_velocity.m_y = 0.0f;
}

bool CollisionTest::colliding(Vector2& position)
{
    collisionBox.m_origin = position;
    collisionBox.m_origin.m_y -= (PLAYERHEIGHT * 0.5f);
    collisionBox.m_halfExtents.Set(PLAYERWIDTH * 0.5f, PLAYERHEIGHT * 0.5f);
    ...
    for (int y = minTileY; y < maxTileY; ++y)
    {
        ...
        {
            int tile = m_mapData.GetTile(GameMap::kPlayground, x, y);
            if (tile != GameMap::kEmptyTile)
            {
                return true;
            }
